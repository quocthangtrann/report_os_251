...
\begin{enumerate}[label=\textbf{\arabic*.}]
	...
	\begin{enumerate}[label=\textbf{\alph*.}]
		...
		Additional levels provide more granular control over memory management, allowing the operating system to manage different regions of the address space with higher utilization and more efficient allocation.
	\end{enumerate}
...
\begin{enumerate}[label=\textbf{\arabic*.}]
	...
	\begin{enumerate}[label=\textbf{\alph*.}]
		...
		\begin{itemize}
			...
			\item \textbf{Race Conditions:} Concurrent calls to \texttt{pte\_set\_fpn()} and \texttt{pte\_get\_entry()} could result in reading partially updated page table entries, causing incorrect address translations and memory access violations.
