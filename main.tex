...
The scheduler module is the core component responsible for orchestrating process execution. In this assignment, we implement a scheduler for a \textbf{Symmetric Multiprocessing (SMP)} system using a \textbf{Slot-based Multi-Level Queue (MLQ)} algorithm. This design balances the strictness of priority scheduling with fairness mechanisms required to prevent starvation, as mandated by the project specifications.

\begin{enumerate}
    \item \textbf{Multilevel Queue Scheduling (Partitioning):}
    According to OS theory, a multilevel queue scheduling algorithm partitions the ready queue into several separate queues. In our system, processes are permanently assigned to a specific queue based on their fixed priority (\texttt{prio}). The system defines \texttt{MAX\_PRIO} levels, ranging from 0 (highest priority) to \texttt{MAX\_PRIO - 1} (lowest priority).

    \item \textbf{Intra-Queue Scheduling (Round-Robin):}
    Within a single priority queue, the assignment specifies that the CPU must run processes in a \textbf{Round-Robin (RR)} style.
    \begin{itemize}
        \item Each process is assigned a \textit{Time Slice} (defined in the system configuration).
        \item When the time slice expires, if the process has not finished, the CPU is forced to preempt it.
        \item The preempted process is placed back at the \textbf{tail} of its priority queue, waiting for its next turn. This necessitates a First-In-First-Out (FIFO) queue structure.
    \end{itemize}

    \item \textbf{Inter-Queue Scheduling (Slot-based Allocation):}
    A critical issue in fixed-priority scheduling is \textit{starvation}, where low-priority processes effectively never execute. To mitigate this, the textbook suggests allocating CPU time among queues:
    \begin{equation}
        slot[i] = MAX\_PRIO - i
    \end{equation}
    Each queue $i$ is given a budget of $slot[i]$ dispatches. The scheduler iterates through queues; if a high-priority queue exhausts its slots, the scheduler \textbf{must} yield to the next priority level, even if the high-priority queue is not empty. This ensures lower-priority queues eventually receive CPU cycles.

    \item \textbf{Symmetric Multiprocessing (SMP) \& Load Balancing:}
    The system simulates 2 CPUs sharing a \textbf{Common Ready Queue} (Global Queue). According to SMP theory, this architecture allows for automatic \textit{Load Balancing} and \textit{Process Migration}. 
    \begin{itemize}
        \item Any available CPU checks the global \texttt{mlq\_ready\_queue} to pick the next process.
        \item A process running on CPU 0 may be paused and later resumed on CPU 1.
        \item To prevent race conditions (e.g., two CPUs picking the same process), we employ a Mutex Lock (\texttt{queue\_lock}) to protect the critical section during queue access.
    \end{itemize}
\end{enumerate}
...
Although the virtual address space is linear, logically, a process is typically organized into distinct sections to manage access rights and usage purposes (referencing \textbf{Chapter 9.3}):

\begin{itemize}
    \item \textbf{Text Section:} Contains the executable code of the program (typically Read-Only).
    \item \textbf{Data Section:} Contains global and static variables.
    \item \textbf{Heap:} The memory region for dynamic allocation, which grows upward in the address space.
    \item \textbf{Stack:} Contains stack frames for function calls and local variables, which grows downward.
\end{itemize}

The operating system manages these segments as \textbf{Virtual Memory Areas (VMAs)}. Each VMA is characterized by a start address, an end address, and permission flags (read/write/execute).
...
\paragraph{a. Hierarchical Paging}
	...
	\begin{figure}[H] 
		...
		\caption{Hierarchical Page Tables Diagram}
	\end{figure}
	...
	\begin{itemize}
		\item \textbf{Memory Efficiency for Sparse Addresses:} The system does not need to allocate memory for the entire page table at once. It solves the problem where a single-level table would require an enormous amount of contiguous memory. It allows dividing the page table into smaller units.
		 \item \textbf{Hardware Support:} This is the standard implementation for modern architectures, such as the Intel x86-64 (which uses a 4-level hierarchy) and ARMv8 architectures.
	\end{itemize}
	
	\noindent \textbf{Disadvantages}
	\begin{itemize}
		 \item \textbf{Memory Access Latency:} As the address space expands (e.g., to 64-bit), the number of levels in the hierarchy increases. Without a TLB, a single memory reference might require multiple memory accesses (e.g., 4 accesses) to traverse the page table before accessing the actual data.
	...
	\begin{figure}[H]
		...
		\caption{Hashed Page Table Diagram}
	\end{figure}
	...
	\paragraph{c. Inverted Page Tables}
	 ...
	 \begin{figure}[H]
	 	\centering
	 	\includegraphics[width=1\textwidth]{Inverted Page Tables}
	 	\caption{Inverted Page Table Diagram}
	 \end{figure}
	...
	In this simple operating system simulation project, we selected Hierarchical Page Tables for the following three reasons:
	...
	\begin{itemize}
		...
		Compared to Inverted Page Tables or Hashed Page Tables, the hierarchical structure is easier to implement within a C-based simulation environment. We primarily rely on bitwise shifts and array indexing to decompose virtual addresses into their respective indices (e.g., \texttt{pgd\_idx}, \texttt{p4d\_idx}, etc.).
	\end{itemize}
...
The objective of this module is to implement memory management at the \textbf{Logical Level} (Process View), adhering to the principles outlined in Chapter 9 of \textit{Silberschatz}. In this simulation, each process possesses a distinct Logical Address Space, which is segmented into functional areas (VMAs - Virtual Memory Areas) such as Code, Data, Heap, and Stack.

The core design principles enforced in this implementation are:
\begin{itemize}
    \item \textbf{VMA Management:} Organizing virtual memory regions as a linked list of \texttt{vm\_area\_struct}.
    \item \textbf{Separable User/Kernel Space:} A strict isolation between User Mode and Kernel Mode. User programs are prohibited from directly accessing Kernel data structures (such as \texttt{pcb\_t} or \texttt{mm\_struct}). All interactions must occur via System Calls using only the Process ID (\texttt{PID}) as an identifier.
    \item \textbf{Allocation Strategy:} The system prioritizes the \textit{Reuse} of freed memory holes (via \texttt{vm\_freerg\_list}) before performing \textit{Expansion} of the address space to minimize external fragmentation.
\end{itemize}
...
\textbf{1. VMA Data Structure (\texttt{mm.h})} \\
...
\begin{itemize}
    ...
    \item \textbf{Trap to OS:} \texttt{libsyscall} passes the Process ID (\texttt{PID}) and the operation code (e.g., \texttt{SYSMEM\_MAP\_OP}) to the Kernel via a software interrupt/trap.
...
\begin{itemize}
    \item \textbf{Activity:} The function \texttt{get\_vma\_by\_num} traverses the linked list \texttt{mm->mmap}.
    \item \textbf{Purpose:} It locates the \texttt{vm\_area\_struct} with a \texttt{vm\_id} matching the request (e.g., \texttt{vm\_id = 1} for the Data segment). This enforces the logical separation between data, code, and stack.
\end{itemize}
...
The \texttt{pg\_getpage} function is the core of translation, traversing 5 table levels.
...
\begin{lstlisting}[language=C, caption=20-Bit Address Space]
	// common.h
	/* Address geometry (two-level page tables):
	* ADDRESS_SIZE: total VA bits.
	* OFFSET_LEN:   bits within a page (page size = 1 << OFFSET_LEN).
	* FIRST_LV_LEN: bits used by first-level index (L1).
	* SECOND_LV_LEN:bits used by second-level index (L2).
	* SEGMENT_LEN/PAGE_LEN: aliases to L1/L2 lengths.
	*/
...
\begin{lstlisting}[language=C, caption=64-Bit 5 Level Paging]
	// mm64.h
	#define MM64_BITS_PER_LONG 64
	
	#define PAGING64_CPU_BUS_WIDTH 57 /* 57-bit canonical VA space */
...
\end{lstlisting}
	...
	\begin{lstlisting}[language=C, caption=get\_pd\_from\_address() in mm64.c]
		// mm64.c
		/*
	...
	Each address translation requires traversing all 5 levels (PGD $\rightarrow$ P4D $\rightarrow$ PUD $\rightarrow$ PMD $\rightarrow$ PT), requiring 5 memory accesses instead of 2. The implementation in \texttt{get\_page\_table\_entry()} (Listing 12) demonstrates this hierarchical traversal. Modern CPUs use Translation Lookaside Buffers (TLB) to cache recent translations, mitigating this overhead in practice.
	...
	\end{itemize}
	\begin{lstlisting}[language=C, caption=get\_next\_level() in mm64.c]
		// mm64.c
		/*
		* Helper: Get or Allocate Next Level (Directory Navigation)
		*/
		void *get_next_level(uint64_t *entry, int alloc)
		{
			/* Check Real Hardware Present Bit (Bit 0) */
			if (*entry & X86_PRESENT)
			{
				/* Mask off the bottom 12 bits to get the pointer */
				return (void *)(*entry & X86_ADDR_MASK);
			}
			
			if (!alloc)
			return NULL;
			
			/* Allocate 4KB Aligned Table */
			void *new_table = alloc_aligned_table();
			if (new_table == NULL)
			return NULL;
			
			/* * Write the Entry:
			* Pointer (Already 4KB aligned, so bottom 12 bits are 0)
			* OR with X86 Flags (Present + Write + User)
			*/
			*entry = (uint64_t)new_table | X86_PRESENT | X86_WRITE | X86_USER;
	...
	\begin{itemize}
		...
		\begin{lstlisting}[language=C, caption=pte\_set\_fpn() in mm64.c]
		// mm64.c
		/*
		...
		*/
		int pte_set_fpn(struct pcb_t *caller, addr_t pgn, addr_t fpn)
		{
			...
			uint64_t *pte = get_page_table_entry(caller->mm, addr, 1);
			if (pte == NULL)
			return -1;
			
			*pte = 0; // Clear old data
			
			// Mark page present in RAM
			SETBIT(*pte, PAGING_PTE_PRESENT_MASK);
			CLRBIT(*pte, PAGING_PTE_SWAPPED_MASK);
		...
		\begin{lstlisting}[language=C, caption=pte\_set\_swap() in mm64.c]
		// mm64.c
		/*
		...
		*/
		int pte_set_swap(struct pcb_t *caller, addr_t pgn, int swptyp, addr_t swpoff)
		{
			...
			*pte = 0; // Clear old data
			...
			// Swapped pages keep Present=0 and only carry swap metadata
			SETBIT(*pte, PAGING_PTE_SWAPPED_MASK);
		...
		\begin{lstlisting}[language=C, caption=pte\_get\_entry() in mm64.c]
		// mm64.c
		/* Get PTE page table entry */
		uint64_t pte_get_entry(struct pcb_t *caller, addr_t pgn)
		{
			addr_t addr = pgn << PAGING64_ADDR_PT_SHIFT;
			uint64_t *pte = get_page_table_entry(caller->mm, addr, 0);
			if (pte == NULL)
			return 0;
			return *pte;
		}
		...
		\begin{lstlisting}[language=C, caption=vmap\_pgd\_memset() in mm64.c]
		// mm64.c
		/*
		...
		*/
		int vmap_pgd_memset(struct pcb_t *caller, addr_t addr, int pgnum)
		{
			...
			{
				addr_t current_addr = addr + (i * PAGING64_PAGESZ);
				uint64_t *pte = get_page_table_entry(caller->mm, current_addr, 1);
				if (pte)
				{
					*pte = 0; // Clear the LEAF entry (Not Present)
				}
			}
			return 0;
		}
	...
	\begin{itemize}
		...
		\begin{lstlisting}[language=C, caption=init\_mm() in mm64.c]
		// mm64.c
		/* init_mm */
		...
		\end{lstlisting}
		\item \texttt{get\_page\_table\_entry()}: Implements recursive tree traversal with dynamic allocation (Listing 12)
		\item \texttt{print\_pgtbl\_recursive()} (lines 546-595): Recursive function to traverse and print all 5 levels
		\begin{lstlisting}[language=C, caption=print\_pgtbl\_recursive() in mm64.c]
		void print_pgtbl_recursive(void *table_ptr, int level, addr_t current_va)
		{
			if (table_ptr == NULL)
			return;
			
			// Base case: Level 5 (PT), entries are actual PTEs
			if (level == 5)
			{
				struct pt_t *pt = (struct pt_t *)table_ptr;
				for (int i = 0; i < 512; i++)
				{
					// Check LEAF PRESENT Bit (31)
					if (pt->entries[i] & PAGING_PTE_PRESENT_MASK)
					{
						printf("%016llx: %016llx\n", current_va | ((addr_t)i << 12), pt->entries[i]);
					}
				}
				return;
			}
			
			// Recursive case: Levels 1-4 (PGD, P4D, PUD, PMD)
			uint64_t *entries = (uint64_t *)table_ptr;
			int shift = 0;
			switch (level)
			{
				case 1:
			...
			for (int i = 0; i < 512; i++)
			{
				// Check DIRECTORY PRESENT Bit (0)
				if (entries[i] & PAGING_PRESENT)
				{
					void *next_table = EXTRACT_NEXT_ADDR(entries[i]);
					addr_t next_va = current_va | ((addr_t)i << shift);
					print_pgtbl_recursive(next_table, level + 1, next_va);
				}
			}
		}
		\end{lstlisting}
		\item Address extraction in \texttt{get\_pd\_from\_address()}: Complex bit manipulation across 5 levels (Listing 16)
	\end{itemize}
	...
	\begin{lstlisting}[language=C, caption=Directory Structs defined in os-mm.h]
	// os-mm.h
	#define MM_PAGING
	...
	#define PAGING64_ENTRY_COUNT 512 // 9 bits per level = 512 entries
	...
	*/
	...
	// Page Table (Level 1) - Points to Physical Frame
	struct pt_t
	{
		uint64_t entries[PAGING64_ENTRY_COUNT];
	};
	
	// Page Middle Directory (Level 2) - Points to PT
	struct pmd_t
	{
		uint64_t entries[PAGING64_ENTRY_COUNT];
	};
	
	// Page Upper Directory (Level 3) - Points to PMD
	struct pud_t
	{
		uint64_t entries[PAGING64_ENTRY_COUNT];
	};
	
	// Page Level-4 Directory (Level 4) - Points to PUD
	struct p4d_t
	{
		uint64_t entries[PAGING64_ENTRY_COUNT];
	};
	
	// Page Global Directory (Level 5) - Points to P4D
	struct pgd_t
	{
		uint64_t entries[PAGING64_ENTRY_COUNT];
	};
	...
	\begin{lstlisting}[language=C, caption=vmap\_page\_range in os-mm.h]
		/*
...
\end{lstlisting}
	...
	\begin{itemize}
		\item Calculates page-aligned size using \texttt{DIV\_ROUND\_UP(inc\_amt, PAGING\_PAGESZ)} (line 124)
		...
		\item Updates the \texttt{sbrk} pointer (line 158) to track the new segment boundary
	\end{itemize}
	...
	The \texttt{vm\_freerg\_list} within each VM area tracks free regions, allowing efficient reuse of freed memory within segments. The \texttt{get\_free\_vmrg\_area()} function (lines 384-437 of \texttt{libmem.c}) searches the free list for regions large enough to satisfy allocation requests, reducing internal fragmentation. The symbol table (\texttt{symrgtbl}) with \texttt{PAGING\_MAX\_SYMTBL\_SZ = 30} entries enables tracking of variables within logical segments, and the \texttt{\_\_alloc()} function (lines 39-101) first attempts to reuse free regions before expanding the segment.
	
	\item \textbf{Support for Dynamic Growth:}  
	The \texttt{sbrk} pointer in each VM area allows segments to grow dynamically. The \texttt{get\_vm\_area\_node\_at\_brk()} function (lines 47-67 of \texttt{mm-vm.c}) creates new regions starting at the current \texttt{sbrk} position. When \texttt{get\_free\_vmrg\_area()} fails (line 45 of \texttt{libmem.c}), the system expands the heap via \texttt{inc\_vma\_limit()}, which calls \texttt{vm\_map\_ram()} to allocate and map new pages. This enables processes to extend their segments on demand without pre-allocation.
    \end{enumerate}
	...
	Address translation requires two conceptual steps: first identifying the VM area (segment) that contains the address, then translating the address through the page table hierarchy. In practice, the implementation uses the page table directly (via \texttt{get\_page\_table\_entry()} in \texttt{mm64.c}), but VM area validation adds overhead. In 64-bit mode, address translation involves extracting indices from PGD, P4D, PUD, PMD, and PT levels (as in \texttt{get\_pd\_from\_address()}, lines 149-157 of \texttt{mm64.c}), requiring 5 memory accesses. Additionally, functions like \texttt{\_\_read()} (lines 279-290 of \texttt{libmem.c}) must first validate the address is within the correct VM area region via \texttt{get\_symrg\_byid()} before performing page translation.
	
	\item \textbf{Memory Overhead for Management Structures:}  
	Each process maintains a linked list of VM areas (\texttt{mm\_struct->mmap}, line 136 of \texttt{os-mm.h}), each with its own free region list (\texttt{vm\_freerg\_list}). Additionally, the hierarchical page table structure (\texttt{mm\_struct->pgd} in 64-bit mode) must be maintained, with each directory level consuming 4KB when allocated. The \texttt{symrgtbl} array (\texttt{PAGING\_MAX\_SYMTBL\_SZ = 30}, line 139) adds further overhead for tracking memory regions within segments. Each VM area structure itself consumes memory, and the free region lists within each VM area add additional overhead compared to a pure paging system.
	
	\item \textbf{Complex Overlap Validation:}  
	The \texttt{validate\_overlap\_vm\_area()} function (lines 76-102 of \texttt{mm-vm.c}) must check all VM areas to prevent overlaps, requiring traversal of the entire \texttt{mmap} linked list. This adds O(n) overhead during memory allocation operations, as seen in \texttt{inc\_vma\_limit()} (line 138) and \texttt{\_\_alloc()} when expanding segments. The overlap check logic (lines 93-96) compares the new region against all existing VM areas, which becomes expensive as the number of VM areas grows.
	
	\item \textbf{Page Table Management Complexity:}  
	The \texttt{mm\_struct} must maintain both VM area structures and page table structures simultaneously. In 64-bit mode, the structure contains the root PGD pointer (\texttt{mm->pgd}), and the initialization in \texttt{init\_mm()} (lines 358-386 of \texttt{mm64.c}) shows the complexity of managing both segment-like VM areas (initializing \texttt{mmap}, \texttt{symrgtbl}) and hierarchical page tables (initializing \texttt{pgd} as NULL). The \texttt{vmap\_page\_range()} function must walk the 5-level page table hierarchy while also tracking which VM area the pages belong to, adding complexity to memory mapping operations.
	...
	The \texttt{get\_free\_vmrg\_area()} function (lines 384-437 of \texttt{libmem.c}) must traverse the free region list within a VM area to find suitable regions, which can be O(n) in the number of free regions. When regions are freed via \texttt{\_\_free()} (lines 104-139), the function must create new free region nodes and insert them into the list (line 135), potentially fragmenting the free list. The \texttt{enlist\_vm\_freerg\_list()} function (lines 13-27) adds regions to the head of the list, which is efficient but doesn't merge adjacent free regions, potentially leading to fragmentation.   
	 \end{enumerate}
...
\end{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Deadlock in Timer:} The condition variable waits in \texttt{timer\_routine()} (lines 36-38) and \texttt{next\_slot()} (lines 79-81) could deadlock if mutexes are not properly acquired, freezing the entire system.
...
\end{itemize}
    ...
    To demonstrate the consequences of missing synchronization, we removed all mutex protection from \texttt{timer\_routine()} (lines 20-64 of \texttt{timer.c}) and \texttt{next\_slot()} (lines 66-83) functions. This eliminates the critical synchronization mechanism that coordinates time slot advancement across all CPU threads and the loader thread.
...
\end{itemize}
This allows multiple threads (CPU threads from \texttt{cpu\_routine()} in \texttt{os.c} and the loader thread from \texttt{ld\_routine()}) to access and modify the timer state (\texttt{\_time}, line 15 of \texttt{timer.c}) and device \texttt{done} flags concurrently without any protection.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Deadlock if threads wait on condition variables without proper mutex protection (the condition variable waits in \texttt{timer\_routine()} lines 35-37 and \texttt{next\_slot()} lines 78-80 require mutexes to function correctly)
