...
\begin{itemize}
	...
	\item \textbf{Huge Page (2MB):} The CPU only traverses 4 levels. At the level 1 (Page Directory - PT), a special bit (PS bit - Page Size) is set. The entry in the PT points directly to a \textbf{2MB physical frame} instead of a Level-5 Page Table.
	\item \textbf{Huge Page (1GB):} The CPU only traverses 3 levels. At the level 2 (Page Directory Pointer - PMD), the PS bit is set, pointing directly to a \textbf{1GB physical frame}.
\end{itemize}
