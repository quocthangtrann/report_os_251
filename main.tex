...
\usepackage{ragged2e} % Sử dụng gói này để có lệnh \justify
\usepackage[backend=biber,style=numeric]{biblatex}
\addbibresource{references.bib}
\usepackage{setspace} % Gói để điều chỉnh khoảng cách giữa các dòng
...
\usepackage{lastpage}
\usepackage{longtable}
\usepackage{framed}  % Thêm vào phần preamble của tài liệu
\addbibresource{references.bib}
\usepackage{enumitem}
...
% Add this to make display math the same size as text
\AtBeginDocument{
...
% Header
\fancyhead[L]{\includegraphics[height=1.5cm]{logobk.png}} % Thay 'logo.png' bằng tên file logo của bạn
...
\end{itemize}
Each VM area is then paged using the hierarchical page table structure (\texttt{mm\_struct->pgd} in 64-bit mode, as defined in \texttt{os-mm.h} line 131), where pages are mapped to non-contiguous physical frames. The symbol table (\texttt{symrgtbl}, line 139) tracks memory regions (variables) within VM areas, similar to how segments track logical units. The \texttt{mm\_struct->mmap} field (line 136) maintains a linked list of all VM areas for a process.
	...
	In \texttt{vm\_map\_ram()} (lines 322-337 of \texttt{mm64.c}) and \texttt{vmap\_page\_range()} (lines 251-278), physical memory is allocated in fixed-size 4KB frames. The \texttt{framephy\_struct} linked list (defined in \texttt{os-mm.h} lines 155-162) manages non-contiguous frames, eliminating external fragmentation entirely since segments are paged. The \texttt{alloc\_pages\_range()} function (lines 283-317) allocates frames individually and links them, allowing a VM area to span multiple non-contiguous physical frames.
