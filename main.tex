...
%	\end{itemize}
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%
	\noindent \textbf{b. Hashed Page Tables}
...
%	\end{itemize}
	...
	\noindent \textbf{Variation: Clustered Page Tables}
	
	A common variation of this method is \textbf{Clustered Page Tables}. Instead of storing a single page per entry in the list, it stores mappings for multiple contiguous pages. This technique helps reduce the length of the linked list and improves search performance for large memory blocks. 
	\begin{figure}[H]
		\centering
		\includegraphics[width=1\textwidth]{Clustered Page Tables}
...
%	\end{itemize}
	...
	\noindent \textbf{c. Inverted Page Tables}
...
% --- 1. FCFS ---
   ...
   % --- 2. SJF ---
    ...
    \begin{itemize}
        ...
        SJF (or more accurately, \textit{Shortest-Next-CPU-Burst}) is theoretically proven to yield the minimum average waiting time. However, it suffers from a fundamental implementation flaw: the OS cannot know the length of the next CPU burst ($\tau_{n+1}$) in advance.
        
        To implement SJF, the system must rely on \textbf{Exponential Averaging} to predict the future based on past behavior, using the formula:
        \begin{equation}
            \tau_{n+1} = \alpha t_n + (1 - \alpha)\tau_n
        \end{equation}
        Where $t_n$ is the most recent burst and $\tau_n$ is the past history. The parameter $\alpha$ ($0 \le \alpha \le 1$) controls the relative weight of recent versus past history:
        \begin{itemize}
            \item \textbf{Properties of $\alpha$ (Weighting):} 
            The expansion of the formula ($\tau_{n+1} = \alpha t_n + (1-\alpha)\alpha t_{n-1} + \dots$) shows that the weight of each past burst decays exponentially.
            \item \textbf{Scenario Analysis (When is it bad?):} 
            \begin{itemize}
                \item If $\alpha \approx 0$ (History dominates): The system is sluggish. If a process changes behavior (e.g., from I/O-bound to CPU-bound), the scheduler fails to adapt, treating a long job as a short one.
                \item If $\alpha \approx 1$ (Recent dominates): The system overreacts to transient spikes. A single unusually long burst (an outlier) will cause the scheduler to penalize the process significantly in the next turn, even if the long burst was a one-time event.
            \end{itemize}
        \end{itemize}
        Thus, while optimal in theory, SJF is computationally expensive (floating-point math in kernel) and prone to prediction errors when process behavior is unstable.
        
        \item \textbf{Assignment Implementation Analysis (Static Determinism):}
        Our implemented algorithm prioritizes \textbf{Determinism} and \textbf{Feasibility} over theoretical optimality by avoiding runtime predictions entirely.
        \begin{itemize}
            \item \textit{Mechanism:} Instead of guessing $\tau_{n+1}$, the system relies on the explicit \texttt{proc->prio} value defined in the PCB. This value represents the "importance" or "class" of a process rather than its estimated burst length.
            \item \textit{Code Evidence:} The selection logic in \texttt{sched.c} (\texttt{get\_mlq\_proc}) iterates through \texttt{mlq\_ready\_queue[i]} using simple integer comparisons. This avoids the overhead of calculating exponential averages ($O(1)$ arithmetic vs. floating-point operations).
        \end{itemize}
        
        \item \textbf{Comparative Evaluation:} Unlike SJF, which can degrade if $\alpha$ is poorly tuned or if processes exhibit erratic burst patterns (e.g., mixed CPU/IO workloads), our Slot-based MLQ guarantees that high-priority processes always run first, while the slot mechanism prevents the starvation that strict priority scheduling usually suffers from.
    \end{itemize}
